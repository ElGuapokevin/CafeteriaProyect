...
((SELECT idhorario FROM public.horarios WHERE tiempocomida='Desayuno'),   'Desayuno Chapin', 'Huevos + frijol + plátano', 20.00, 50, NULL),
((SELECT idhorario FROM public.horarios WHERE tiempocomida='Desayuno'),   'Panqueques',      'Panqueques con miel',       18.00, 30, NULL),
((SELECT idhorario FROM public.horarios WHERE tiempocomida='Almuerzo'),   'Pollo Asado',     'Pollo con arroz y ensalada',35.00, 40, NULL),
((SELECT idhorario FROM public.horarios WHERE tiempocomida='Almuerzo'),   'Carne Guisada',   'Carne con papas y arroz',   38.00, 35, NULL),
((SELECT idhorario FROM public.horarios WHERE tiempocomida='Cena'),       'Hamburguesa',     'Hamburguesa con papas',     30.00, 25, NULL),
((SELECT idhorario FROM public.horarios WHERE tiempocomida='Refaccion'),  'Licuado Banana',  'Licuado de banano',         12.00, 60, NULL);
